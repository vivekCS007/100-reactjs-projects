...
const FluidCursor = () => {
  ...
  useEffect(() => {
    ...
    function step(dt: number) {
      ...
      curlProgram.bind();
      gl.uniform2f(curlProgram.uniforms.texelSize, velocityLocal.texelSizeX, velocityLocal.texelSizeY);
      gl.uniform1i(curlProgram.uniforms.uVelocity, velocityLocal.read.attach(0));
      blit(curlLocal);

      vorticityProgram.bind();
      gl.uniform2f(vorticityProgram.uniforms.texelSize, velocityLocal.texelSizeX, velocityLocal.texelSizeY);
      gl.uniform1i(vorticityProgram.uniforms.uVelocity, velocityLocal.read.attach(0));
      ...
      gl.uniform2f(divergenceProgram.uniforms.texelSize, velocityLocal.texelSizeX, velocityLocal.texelSizeY);
      gl.uniform1i(divergenceProgram.uniforms.uVelocity, velocityLocal.read.attach(0));
      blit(divergenceLocal);

      clearProgram.bind();
      gl.uniform1i(clearProgram.uniforms.uTexture, pressureLocal.read.attach(0));
      gl.uniform1f(clearProgram.uniforms.value, config.PRESSURE);
      blit(pressureLocal.write);
      pressureLocal.swap();

      pressureProgram.bind();
      gl.uniform2f(pressureProgram.uniforms.texelSize, velocityLocal.texelSizeX, velocityLocal.texelSizeY);
      gl.uniform1i(pressureProgram.uniforms.uDivergence, divergenceLocal.attach(0));
      for (let i = 0; i < config.PRESSURE_ITERATIONS; i++) {
        gl.uniform1i(pressureProgram.uniforms.uPressure, pressureLocal.read.attach(1));
        blit(pressureLocal.write);
        pressureLocal.swap();
      }

      gradienSubtractProgram.bind();
      gl.uniform2f(gradienSubtractProgram.uniforms.texelSize, velocityLocal.texelSizeX, velocityLocal.texelSizeY);
      gl.uniform1i(gradienSubtractProgram.uniforms.uPressure, pressureLocal.read.attach(0));
      gl.uniform1i(gradienSubtractProgram.uniforms.uVelocity, velocityLocal.read.attach(1));
      blit(velocityLocal.write);
      velocityLocal.swap();

      advectionProgram.bind();
      gl.uniform2f(advectionProgram.uniforms.texelSize, velocityLocal.texelSizeX, velocityLocal.texelSizeY);
      if (!ext.supportLinearFiltering) {
        gl.uniform2f(advectionProgram.uniforms.dyeTexelSize, velocityLocal.texelSizeX, velocityLocal.texelSizeY);
      }
      ...
      if (!ext.supportLinearFiltering) {
        gl.uniform2f(advectionProgram.uniforms.dyeTexelSize, dyeLocal.texelSizeX, dyeLocal.texelSizeY);
      }
      gl.uniform1i(advectionProgram.uniforms.uVelocity, velocityLocal.read.attach(0));
    ...
    function updatePointerMoveData(pointer: PointerData, posX: number, posY: number, color: ColorRGB) {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvasEl.width;
      pointer.texcoordY = 1.0 - posY / canvasEl.height;
